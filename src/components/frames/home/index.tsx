    ...
    return (
        <div className={style.main}>
            ...
            <div className="event-box-parent">
                {events.map((el:{_id:string, name:string, description:string, date:string, tags:string[], members:string[]}) => {return <EventInDetails key={el._id} _id={el._id} name={el.name??"Название"} description={el.description??"Описание"} date={el.date??"12.12.22"} tags={el.tags??[]} registered={window.user.data._id != undefined && window.user.data._id in el.members} />})}
            </div>
        </div>
    )
}