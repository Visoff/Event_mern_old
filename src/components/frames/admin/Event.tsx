    ...
    return (
        <div className={style.event+" "+(active ? style.active : "")} onClick={(e) => {setActive(true)}}>
            ...
            <form onSubmit={e => {e.preventDefault(); setActive(false); update(event)}}>
                ...
                <p>Описание: <input type="date" onChange={e => {setEvent({...event, date:new Date(e.target.value).toLocaleDateString()})}} value={new Date(new Date(event.date).toDateString()??"").toISOString().substring(0, 10)} /></p>
