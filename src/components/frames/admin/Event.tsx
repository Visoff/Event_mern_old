...
export default function event(el:{_id:string, name?:string, description?:string}) {
    ...
    return (
        <div className={style.event+" "+(active ? style.active : "")} onClick={(e) => {setActive(true)}}>
            <h1 className={style.name} contentEditable="true" onChange={(e) => {console.log({...event, name:(e.target as HTMLElement).innerText})}}>{event.name??"Название"}</h1>
            <form onSubmit={e => {e.preventDefault(); setActive(false);}}>
                <p>Описание: <input type="text" onChange={e => {console.log({...event, description:e.target.value})}} value={event.description} /></p>
